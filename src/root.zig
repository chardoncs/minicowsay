...
fn parseOptions(input: anytype) CowsayOptions {
    ...
    switch (inputType) {
        .@"struct" => |struct_info| {
            if (@hasField(struct_info, "eyes")) {
                result.eyes = @field(struct_info, "eyes");
            }

            if (@hasField(struct_info, "thought_slash")) {
                result.thought_slash = @field(struct_info, "thought_slash");
            }

            if (@hasField(struct_info, "tongue")) {
...
fn wrapLines(allocator: Allocator, message: *const []const u8) Allocator.Error!LineWrapReturn {
    var line_list = std.ArrayList([]const u8).init();
...
fn matchBoundary(line_count: u32, line_idx: u32, b: u8, top_b: u8, bottom_b: u8, cont_b: u8) u8 {
    return switch (line_count) {
        1 => b,
        else => {
            switch (line_idx) {
                0 => top_b,
                line_count - 1 => bottom_b,
                else => cont_b,
            }
        },
    };
...
fn parseBubble(allocator: Allocator, out_list: *CharArrayList, message: *const []const u8) Allocator.Error!void {
    const width = estimateWidth();
    ...
    while (line_idx < lines.len) : (line_idx += 1) {
        try out_list.append(
            matchBoundary(lines.len, line_idx, LEFT_BOUNDARY, TOP_LEFT_BOUNDARY, BOTTOM_LEFT_BOUNDARY, CONTINUE_BOUNDARY)
        );

        try out_list.appendSlice(lines[line_idx]);

        try out_list.append(
            matchBoundary(lines.len, line_idx, RIGHT_BOUNDARY, TOP_RIGHT_BOUNDARY, BOTTOM_RIGHT_BOUNDARY, CONTINUE_BOUNDARY)
        );
    }
...
pub export fn cowsay(allocator: Allocator, message: ?*const [:0]const u8, opt: anytype) Allocator.Error![:0]u8 {
    const options = try parseOptions(opt);
